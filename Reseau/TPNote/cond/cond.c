...
#define NB_CLIENTS      5
 ...
 //gcc cond.c -o cond -lpthread OU -pthread
...
static void * fn_clients (void * p_data)
{
   ...
   while (1)
   {
      ...
      if (val > store.stock) //le stock ne peut plus etre déduit
      {
         printf("STOCK trop bas\n");
         pthread_cond_signal (& store.cond_stock); //on essai avec les prochains clients ou le magasin
         pthread_cond_wait (& store.cond_clients, & store.mutex_stock); //le stock a changé et est satisfaisant
      }
